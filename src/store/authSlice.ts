...
// //       );
// //       if (response.data.success) {
// //         const { access, profile: user } = response.data.data;
// //         localStorage.setItem('userInfo', JSON.stringify(user));
// //         localStorage.setItem('token_user', access.token);
// //         return { user, token: access.token };
// //       } else {
// //         return rejectWithValue(response.data.message || 'Failed to log in');
// //       }
// //     } catch (error: any) {
...
//         ? 'https://api.allinall.social/api/otz-hrm/auth/super-admin/refresh-token'
//         : 'https://api.allinall.social/api/otz-hrm/auth/refresh-token';
...
export const refreshAccessToken = createAsyncThunk(
  ...
  async ({ isAdmin }: { isAdmin: boolean }, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      // Use the login endpoints instead of a dedicated refresh token endpoint
      const endpoint = isAdmin
        ? 'https://api.allinall.social/api/otz-hrm/auth/super-admin/login'
        : 'https://api.allinall.social/api/otz-hrm/auth/login';

      // Send the refresh token in the request body (adjust based on backend requirements)
      const response = await axios.post(
        endpoint,
        { email, password }, // Assuming the backend expects 'refreshToken' in the body
        { headers: { 'Content-Type': 'application/json' } }
      );
      ...
      if (response.data.success) {
        ...
        if (isAdmin) {
          localStorage.setItem('token', access.token);
          localStorage.setItem('refreshToken', refresh.token); // Update refresh token if provided
          dispatch(setAdminCredentials({ adminToken: access.token, adminRefreshToken: refresh.token }));
        } else {
          localStorage.setItem('token_user', access.token);
          localStorage.setItem('refreshToken_user', refresh.token); // Update refresh token if provided
          dispatch(setUserCredentials({ userToken: access.token, userRefreshToken: refresh.token }));
        }
        return { token: access.token, refreshToken: refresh.token };
      } else {
